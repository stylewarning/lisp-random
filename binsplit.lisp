...
(defun compute-pi (prec)
  (cl-stopwatch:stopwatch-start :sqrt)
  (let* ((chud-c 640320)
         (chud-c/12 (/ chud-c 12))
         (num-terms (floor (+ 2 (/ prec +chud-decimals-per-term+))))
         (sqrt-c (isqrt (* chud-c (expt 100 prec)))))
    (format t "sqrt: ~A~%" (cl-stopwatch:stopwatch-stop :sqrt))
    (cl-stopwatch:stopwatch-start :pi)
    (let ((pi-calculation (compute-series pi-series :upper num-terms)))
      (format t "pi: ~A~%" (cl-stopwatch:stopwatch-stop :pi))
      (cl-stopwatch:stopwatch-start :floor)
      (prog1 (values (floor  (* sqrt-c chud-c/12)
                             pi-calculation))
        (format t "floor: ~A~%" (cl-stopwatch:stopwatch-stop :floor))))))