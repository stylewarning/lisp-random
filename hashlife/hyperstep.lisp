...
(defun hyperstep (mc)
  ...
  (labels ((compute-hyperstep (mc)
           ...
           (hyperstep-general-case (mc)
             (let ((n00 (hyperstep (macrocell-nw mc)))
                   (n01 (horizontal-center* (macrocell-nw mc)
                                            (macrocell-ne mc)))
                   (n02 (hyperstep (macrocell-ne mc)))
                   (n10 (vertical-center* (macrocell-nw mc)
                                          (macrocell-sw mc)))
                   (n11 (center* mc))
                   (n12 (vertical-center* (macrocell-ne mc)
                                          (macrocell-se mc)))
                   (n20 (hyperstep (macrocell-sw mc)))
                   (n21 (horizontal-center* (macrocell-sw mc)
                                            (macrocell-se mc)))
                   (n22 (hyperstep (macrocell-se mc))))
               (make-macrocell
                (hyperstep (make-macrocell n00 n01 n10 n11))
                (hyperstep (make-macrocell n01 n02 n11 n12))
                (hyperstep (make-macrocell n10 n11 n20 n21))
                (hyperstep (make-macrocell n11 n12 n21 n22))))))
