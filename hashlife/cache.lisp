...
(defun macrocell-set-cell (new-value mc x y)
  "Non-destructively set the cell located at (X, Y) of the macrocell MC. This will produce a new tree of cells, with possibly shared nodes.

The coordinate system is defined to start at (-2^(LEVEL - 1), 2^(LEVEL - 1)) in the northwesternmost corner."
  (check-type new-value bit "The new value of the cell being set must be 0 or 1.")
  (labels ((set-it (mc x y &aux (level (macrocell-level mc)))
             (if (= 1 level)
                 (cond
                   ((and (= -1 x) (= -1 y))
                    (make-macrocell new-value
                                    (macrocell-ne mc)
                                    (macrocell-sw mc)
                                    (macrocell-se mc)))
                   ((and (= 0 x) (= -1 y))
                    (make-macrocell (macrocell-nw mc)
                                    new-value
                                    (macrocell-sw mc)
                                    (macrocell-se mc)))
                   ((and (= -1 x) (= 0 y))
                 ...
                 (let ((offset (expt 2 (- level 2))))
                   (if (minusp x)
                       (if (minusp y)
                           (make-macrocell (set-it (macrocell-nw mc) (+ x offset) (+ y offset))
                                           (macrocell-ne mc)
                                           (macrocell-sw mc)
                                           (macrocell-se mc))
                           (make-macrocell (macrocell-nw mc)
                                           (macrocell-ne mc)
                                           (set-it (macrocell-sw mc) (+ x offset) (- y offset))
                                           (macrocell-se mc)))
                       (if (minusp y)
                           (make-macrocell (macrocell-nw mc)
                                           (set-it (macrocell-ne mc) (- x offset) (+ y offset))
                                           (macrocell-sw mc)
                                           (macrocell-se mc))
                           (make-macrocell (macrocell-nw mc)
                                           (macrocell-ne mc)
                                           (macrocell-sw mc)
                                           (set-it (macrocell-se mc) (- x offset) (- y offset)))))))))
    (set-it mc x y)))
